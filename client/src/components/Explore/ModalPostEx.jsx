...
const ModalPostEx = ({ post: currentPost, handleHideModal }) => {
  ...
  const handleSave = () => {
    setSaved((prev) => !prev);
    dispatch(savePostExplore(currentPost?._id));
  };

  const handleLoves = () => {
    dispatch(likePostExplore(currentPost?._id));
